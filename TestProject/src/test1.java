...
public class test1 { 
	...
	public static void main(String[] args) {
		try {
			...
			palindrome(list); //list에 담긴 각 문장이 회문인지 검사하여 결과를 출력한다
		}catch(FileNotFoundException e) { //FileNotFoundException 예외 처리
		}
	}
	
	private static void palindrome(ArrayList<String> list) { //전달받은 String타입 ArrayList의 원소들이 회문인지 아닌지 검사하여 결과를 출력하는 함수
		for(int i=0; i<list.size(); i++) { //list의 첫번째 원소부터 마지막 원소까지 반복한다
			String string1=list.get(i); //해당 차례의 원소(문장)를 string1변수에 저장한다
			...
			if(string1.equals(string2) && string1.length()>1) { //원래의 문장과 거꾸로 배열한 문장이 동일하다면 이는 문장이 회문이라는 것을 뜻한다. 한글자만 적힌 문자열의 경우 회문이 아니라고 판단한다. 
				System.out.println(i+1+"번째 문장은 회문입니다."); //해당 차례의 문장이 회문이라고 프린트한다. 
			}
			else { //원래의 문장과 거꾸로 배열한 문장이 동일하지 않거나 한글자만 적혀 있다면 회문이 아니라고 판단한다. 
				System.out.println(i+1+"번째 문장은 회문이 아닙니다."); //해당 차례의 문장이 회문이 아니라고 프린트한다. 
			}
		}
	}
